...
<body>
	...
	<input type="button" value="쪽지보내기" onclick="location.href='postbox.do?command=writePost&type=sendPostbox&page=${page }'"/>&nbsp;
	<input type="submit" value="삭제"/>
	<br/>	
	<input type="hidden" name="command" value="multiDel"/>
	<input type="hidden" name="type" value="sendPostbox"/>
	...
	<div class="pagination">
		...
		<c:choose>
			<c:when test="${prevPage >0}">
				<a href="postbox.do?command=sendPostbox&page=${prevPage }" class="page gradient" >◀</a>
			</c:when>
			<c:otherwise>
				<a href="postbox.do?command=sendPostbox&page=1" class="page gradient">◀</a>
			</c:otherwise>
		</c:choose>
		<c:forEach begin="${absolutePage }" end="${endPage }" var="i">
			<c:choose>
				<c:when test="${i eq page}">
					<a href="postbox.do?command=sendPostbox&page=${i}"><span class="page active">${i}</span></a>
				</c:when>
				<c:otherwise>
					<a href="postbox.do?command=sendPostbox&page=${i}" class="page gradient">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			<c:when test="${nextPage < totalPage}">
				<a href="postbox.do?command=sendPostbox&page=${nextPage }" class="page gradient">▶</a>
			</c:when>
			<c:otherwise>
				<a href="postbox.do?command=sendPostbox&page=${totalPage}" class="page gradient">▶</a>
			</c:otherwise>
		</c:choose>
		</div>
